...
# metrics measures how good your model is.
...
# This does the same things as the above.
...
logistic_reg() %>%
  ...
  gt() %>% 
    tab_header("Model 2 ver. 2.0 Evaluation on All Data")
```

```{r}
x_split <- initial_split(clean, prop = 0.01)

training_x <- training(x_split)
testing_x <- testing(x_split)

logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote ~ gender + party + state + office, data = training_x) %>%
  predict(new_data = training_x) %>%
  bind_cols(training_x) %>%
  metrics(truth = dvote, estimate = .pred_class) %>%
  gt() %>% 
    tab_header("Model 2 ver. 3.0 Evaluation on Training Data")
```
...
logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote ~ gender + party + state + office, data = training_x) %>%
  predict(new_data = testing_x) %>%
  bind_cols(testing_x) %>%
  metrics(truth = dvote, estimate = .pred_class) %>%
  gt() %>% 
    tab_header("Model 2 ver. 3.1 Evaluation on Testing Data")
```
